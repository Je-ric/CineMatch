...
@php
    ...
    $rating = $movie->avg_rating !== null ? number_format($movie->avg_rating, 1) : 'N/A';
    $year = $movie->release_year ? "({$movie->release_year})" : '';

    $country = $movie->country_name
        ? "<span class='px-2 py-0.5 rounded bg-neutral-800/70 border border-neutral-700'>{$movie->country_name}</span>"
        : '';

    $language = $movie->language_name
        ? "<span class='px-2 py-0.5 rounded bg-neutral-800/70 border border-neutral-700'>{$movie->language_name}</span>"
        : '';

    $editUrl = route('movies.manage.edit', ['id' => $movie->id]);
    $deleteUrl = route('movies.destroy', ['id' => $movie->id]);
@endphp
...
<div
    ...
    <div class="relative">
        ...
        <div class="absolute top-2 right-2">
            <span
                class="bg-green-600/90 text-white font-semibold text-xs px-2 py-1 rounded-md flex items-center gap-1">
    ...
    <div class="p-4 flex flex-col flex-grow">
        <h5 class="font-semibold text-base mb-1 text-white leading-tight">
            {{ $movie->title }} <small class="text-gray-400 font-normal">{{ $year }}</small>
        </h5>
